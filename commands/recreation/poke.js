...
module.exports = {
    ...
    async execute(client, message, args, Discord, profileData) {
        ...
        if(rng <= 10) {
            ...
            return message.channel.send(`||You successfully poke ${receiver.user.username} and sneak away unnoticed...\nOh what a sly fox you are!||`);
        } else if(rng <= 50) {
            ...
            return message.channel.send(`||The sweet scent of baked goods catches you off guard.\nYou follow the aroma to a local bakery.\nUnfortunately, they've just sold out and ${receiver.user.username} is gone when you turn around as well.||`);
        } else if(rng <= 90) {
            return message.channel.send(`||${receiver.user.username} is on their phone again, not paying attention to their suroundings.\nYou find an opportunity to sneak by and poke them in the back.\nIt's unfortunate that they were too preoccupied to even notice.||`);
        } else {
            return message.channel.send(`||You turn to look for ${receiver.user.username}, but a gust of wind passes by and woosh! They're gone!?||`);
        }
    }
}